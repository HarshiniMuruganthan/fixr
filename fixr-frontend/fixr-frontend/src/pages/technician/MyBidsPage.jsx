...
export default function MyBidsPage() {
  ...
  useEffect(() => {
    // Load all repairs, filter bids for this technician
    repairsAPI.getAll().then(async res => {
      const repairs = res.data
      const allBidsRes = await Promise.all(repairs.map(r => bidsAPI.getForRepair(r._id)))
      const myBids = []
      allBidsRes.forEach((bidsRes, i) => {
        const myBid = bidsRes.data.find(b =>
          (b.technician?._id || b.technician) === user._id
        )
        if (myBid) myBids.push({ ...myBid, repair: repairs[i] })
      })
      setBidsWithRepairs(myBids)
                ...
                <div className="flex items-start justify-between gap-2 flex-wrap">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      ...
                      <span className="text-xs text-dark-400 dark:text-dark-500">📍 {b.repair.location}</span>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <div className="text-xs text-dark-400 dark:text-dark-500">Budget: {formatCurrency(b.repair.budget)}</div>
                  </div>
                </div>
                {b.message && (
