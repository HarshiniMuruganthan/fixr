...
export default function TechnicianDashboard() {
  ...
  const { data: jobs = [],        loading: loadingJobs }    = useApi(() => repairsAPI.getAssigned())
  const { data: allRepairs = [],  loading: loadingAll }     = useApi(() => repairsAPI.getAll())
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <div className="animate-in stagger-3">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/browse" className="text-sm text-brand-600 dark:text-brand-400 hover:underline">All →</Link>
          </div>
          {loading ? (
        ...
        <div className="animate-in stagger-4">
          ...
          {loadingJobs ? (
            ...
            <div className="space-y-3">
              {jobs.filter(j => j.status === 'in_progress').map((j, i) => (
